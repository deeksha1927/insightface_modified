...
def get_dataloader(
    ...
    # Image Folder
        ...
        augmentation_transforms = [
            ...
            transforms.Compose([
                transforms.Resize((112, 112)),
                transforms.RandomAffine(degrees=(-40, 40), translate=(0.1, 0.1), shear=10),  # Random affine transformation
                transforms.ToTensor(),
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
            ]),
            transforms.Compose([
            ...
            transforms.Compose([
                transforms.Resize((112, 112)),
                ChannelIncreaseTransform(),  # Add the custom channel increase transform
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
            ]),
          transforms.Compose([
            transforms.Resize((112, 112)),
        ...
        train_set = ImageFolder(root_dir, transform=None)
        print(len(train_set))
        ...
        for image, label in train_set:
            ...
            for transform in augmentation_transforms:
                aug_image = transform(image)  # Apply each augmentation
                augmented_train_data.append((aug_image, label))  # Store the augmented image with its label
...
class GaussianNoiseTransform:
    def __call__(self, img):
        img_tensor = transforms.ToTensor()(img)  # Convert PIL image to tensor
        
        # Add Gaussian noise
        noise = torch.randn(img_tensor.size()) * 0.1
        img_tensor = img_tensor + noise
        
        return torch.clamp(img_tensor, 0, 1)  # Ensure pixel values stay within [0, 1]


class ChannelIncreaseTransform:
    def __call__(self, img):
        # Check if the image is a PIL Image and convert to tensor if necessary
        if isinstance(img, transforms.ToTensor):
            img_tensor = img
        else:
            img_tensor = transforms.ToTensor()(img)

        # Select a random channel
        channel_choice = random.choice([0, 1, 2])  # 0: Red, 1: Green, 2: Blue
        increase_value = random.randint(10, 100) / 255.0  # Normalize to [0, 1]
        
        # Increase the selected channel
        img_tensor[channel_choice, :, :] += increase_value
        
        # Clamp values to ensure they stay in the range [0, 1]
        img_tensor[channel_choice, :, :] = torch.clamp(img_tensor[channel_choice, :, :], 0, 1)
